...
public class ElevatorSimulator {
    ...
    public class Passenger {  //乘客类
         ...
         * @param abortingTime 放弃等待时间
         * @param inFloor      到达楼层
         * @param outFloor     目标楼层
         */
        public Passenger(int arrivingTime, int abortingTime, int inFloor, int outFloor) {
    ...
    protected class PassengerEvent extends Event {
        ...
        PassengerEvent(Passenger passenger, Car car, int occurTime, PassengerEventType type) {
    ...
    protected class CarEvent extends Event {
        ...
        CarEvent(Car car, int occurTime, CarEventType type) {
            this.car = car;
            this.occurTime = occurTime;
            this.type = type;
        }
    ...
    public ElevatorSimulator(int floors, int base, int parallelNumber, int maxLoad,
        ...
        cars = new Car[parallelNumber];
        for (int i = 0; i < parallelNumber; i++) {
            cars[i] = new Car(maxLoad);
        }
    ...
    private void arrive(Passenger p) {
        ...
        for (var ctmp : cars) {  //寻找开着的，方向正确的，未满的电梯
    ...
    private void enter_start(Passenger p, Car c) {
        if (logEnable) {
            System.out.println(time + "\t: Passenger " + p.ID + " enter car.");
        }
    ...
    private void enter_end(Passenger p, Car c) {
    ...
    private void exit_start(Passenger p, Car c) {
        if (logEnable) {
            System.out.println(time + "\t: Passenger " + p.ID + " exit car.");
        }
    ...
    private void exit_end(Passenger p, Car c) {
        c.IOPassenger = null;
    ...
    private void open_end(Car c) {
        c.setState(CarState.detecting);
        events.add(new CarEvent(c, time + detectGap, CarEventType.detect));
        var nextp = c.getNextPassengerForExiting();
        if (nextp == null) {
            nextp = c.getNextPassengerForEntering();
            if (nextp != null) {
    ...
    private void close_start(Car c) {
    ...
    private double FindPosition(Car c, Direction direction, Direction searchDirection) {
        //向上的电梯找最低
        //向下的电梯找最高
    ...
    private boolean searchUp(Car c) {
        //找到c以上向上的最低电梯
...
//        }
    ...
    private boolean searchDown(Car c) {
        //找到c以下向下的最高电梯
...
//        }
     ...
     */
    //todo 待改进
    private void decide_direction(Car c) {
        if (c.direction == Direction.up) {
    ...
    private void half_up_start(Car c) {
        c.setDirection(Direction.up);
        c.setState(CarState.upping);
    ...
    private void half_down_start(Car c) {
        c.setDirection(Direction.down);
        c.setState(CarState.downing);
    ...
    private void idle_up_start(Car c) {
    ...
    private void idle_up_end(Car c) {
        if (c.state == CarState.idle_upping) {
            c.nowFloor++;
            if (c.nowFloor == base) {
                events.add(new CarEvent(c, time, CarEventType.idle_start));
    ...
    private void idle_down_start(Car c) {
    ...
    private void idle_down_end(Car c) {
        if (c.state == CarState.idle_downing) {
            c.nowFloor--;
            if (c.nowFloor == base) {
                events.add(new CarEvent(c, time, CarEventType.idle_start));
